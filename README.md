...
### PK 가 없는 InnoDB 테이블은 어떻게 클러스터링 테이블로 구성될까?
1. PK가 있으면 기본적으로 PK를 클러스터링 키로 선택
2. NOT NULL 옵션의 UNIQUE INDEX 중에서 가장 첫 번째 인덱스를 클러스터링 키로 선택
3. 자동으로 유니크한 값을 가지도록 증가되는 컬럼을 내부적으로 추가한 후, 클러스터링 키로 선택

즉, 적합한 녀석이 없으면 강제로 만들어서라도 클러스터링 키를 구성한다.
그러나 이럴 경우 InnoDB 테이블에서 클러스터링 인덱스로 얻는 혜택이 아무것도 없으므로, 단순 
AUTO-INCREMENT한 PK만 명시적으로 생성해두는 것이 좋다.

### 가능하다면 PK는 AUTO-INCREMENT 보다 비즈니스적인 칼럼으로
클러스터링 인덱싱 과정에서 PK가 비즈니스적으로 의미를 가지면 인덱스 조회시 큰 효과를 누릴 수 있다. 
...
`EXPLAIN` 명령과 달리 실행 계획만 추출하는게 아니라, 실제 쿼리를 실행하고 사용된 실행 계획과 소요된 시간을 보여준다.
쿼리의 실행 계획이 아주 나쁠것이 예상되는 경우 우선 `EXPLAIN`로 쿼리 튜닝 후 `EXPLAIN ANALYZE` 기능을 통해 세부 튜닝을 진행하면 좋다.

> 이번 미션에서 쓰인 MySQL은 5.7.32 버전으로, `EXPLAIN ANALYZE` 기능을 사용할 수 없었다. 아쉽다!!
